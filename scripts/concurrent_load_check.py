...
This is a standalone script, not a pytest test file -- deliberately
named and structured to avoid pytest's *_test.py/test_*.py discovery
patterns (a previous version, concurrent_load_test.py, matched that
pattern exactly, and its internal functions were also named test_* --
bare `pytest` with no arguments recursively searches the whole project
by default, not just tests/, so it collected and ran these as isolated
pytest functions, without ever executing the __main__ block below that
actually starts the server. Every request then failed with a connection
error, since no server was running -- not a bug in this script's actual
logic, just an accidental name collision with pytest's conventions).
pyproject.toml's testpaths = ["tests"] now prevents this at the
configuration level too, but this file is renamed regardless, so its
own name doesn't invite the same mistake again.

Run with: python scripts/concurrent_load_check.py
"""
import subprocess
import sys
import time
...
def check_concurrent_ingestion(n=30):
    print(f"\n--- Check 1: {n} concurrent /ingest calls, each a unique document ---")
...
def check_concurrent_queries(n=30):
    print(f"\n--- Check 2: {n} concurrent /query calls against the populated index ---")
...
def check_mixed_concurrent_load(n_ingest=15, n_query=15):
    print(f"\n--- Check 3: {n_ingest} /ingest + {n_query} /query, all firing at once ---")
...
if __name__ == "__main__":
    ...
    try:
        ...
        result1 = check_concurrent_ingestion(n=30)
        result2 = check_concurrent_queries(n=30)
        result3 = check_mixed_concurrent_load(n_ingest=50, n_query=50)

        print("\n" + "=" * 60)
        print("SUMMARY")
        print("=" * 60)
        print(f"Check 1 (concurrent ingestion): {result1['n_errors']} errors, "
              f"final size {result1['final_index_size']} "
              f"(started at {initial_stats['index_size']}, added {result1['n_requested']})")
        print(f"Check 2 (concurrent queries): {result2['n_errors']} errors, "
              f"{result2['n_malformed']} malformed responses")
        print(f"Check 3 (mixed load): {result3['n_errors']} errors")
    finally:
